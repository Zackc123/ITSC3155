...
def read_all_sandwiches(db: Session = Depends(get_db)):
    return sandwiches.read_all_sandwiches(db=db)
...
def read_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    return sandwiches.read_sandwich(db=db, sandwich_id=sandwich_id)
...
def read_all_resources(db: Session = Depends(get_db)):
    return resources.read_all_resources(db=db)

@app.get("/resources/{resource_id}", response_model=schemas.Resource, tags=["Resources"])
def read_resource(resource_id: int, db: Session = Depends(get_db)):
    return resources.read_resource(db=db, resource_id=resource_id)
...
def read_all_recipes(db: Session = Depends(get_db)):
    return recipes.read_all_recipes(db=db)

@app.get("/recipes/{recipe_id}", response_model=schemas.Recipe, tags=["Recipes"])
def read_recipe(recipe_id: int, db: Session = Depends(get_db)):
    return recipes.read_recipe(db=db, recipe_id=recipe_id)
...
def read_all_order_details(db: Session = Depends(get_db)):
    return order_details.read_all_order_details(db=db)
...
def read_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    return order_details.read_order_detail(db=db, order_detail_id=order_detail_id)
